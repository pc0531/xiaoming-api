...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
            ...
            self.el.search = self.box.querySelector(classSelector('search-content'));
			...
			if(self.box.querySelector(classSelector('province-scroll'))){
                self.el.pvScroll = self.box.querySelector(classSelector('province-scroll'));
                self.el.pvScrollTop = self.box.querySelector(classSelector('province-top'));
                // self.el.selfSearch = self.box.querySelector('.js-search-height');
			}
		...
		caleLayout: function() {
            ...
            if(self.box.querySelector(classSelector('province-scroll'))){
                self.el.pvScroll.style.height = withoutSearchHeight;
			}
			self.el.inner.style.height = withoutSearchHeight;
			var barItemHeight = ((self.box.offsetHeight ) / self.el.barItems.length) + 'px';
            // var barItemHeight = ((self.box.offsetHeight - self.el.selfSearch.offsetHeight)/ self.el.barItems.length) + 'px';
            console.log(self.el.barItems.length);
		...
		scrollTo: function(group) {
            var self = this;
            if(group == 'hot' || group == '热门'){
                self.el.inner.scrollTop = 0;
                return;
            }
			...
			// self.el.pvScroll.scrollTop = groupElement.offsetTop + self.el.pvScrollTop.offsetHeight;
            // self.el.inner.scrollTop = groupElement.offsetTop;
		},
		bindBarEvent: function() {
			...
			var findStart = function(event) {
				...
				if (pointElement) {
					var group = pointElement.innerText == "热门" ? 'hot' : pointElement.innerText;
					if (group && (group.length == 1 ||　group.length == 3)) {
