(function($, window) {
    ...
    var template = '<div id="{{id}}" class="mui-slider mui-preview-image mui-fullscreen"><div class="mui-preview-header"></div><div class="mui-slider-group"></div><div class="mui-preview-footer mui-hidden">{{footer}}</div><div class="mui-preview-loading"><span class="mui-spinner mui-spinner-white">test</span></div></div>';
    var itemTemplate = '<div class="mui-slider-item mui-zoom-wrapper {{className}}"><div class="mui-zoom-scroller"><video src="{{src}}"  autoplay="autoplay" data-preview-lazyload="{{lazyload}}" width="100%" height="100%"></video></div></div>';
    var defaultGroupName = '__DEFAULT';
    var div = document.createElement('div');
    var imgId = 0;
    var PreviewVideo = function(options) {
        this.options = $.extend(true, {
            id: '__MUI_PREVIEWVIDEO',
            zoom: true,
    ...
    var proto = PreviewVideo.prototype;
    proto.init = function() {
    ...
    proto.initEvent = function() {
        var self = this;
        $(document.body).on('tap', 'video[data-previewVideo-src]', function() {
            self.open(this);
            return false;
        });
        var laterClose = null;
    ...
    proto.addImages = function(group, index) {
        ...
        if (group) {
            if (group === defaultGroupName) {
                imgs = document.querySelectorAll("video[data-previewVideo-src]:not([data-preview-group])");
            } else {
                imgs = document.querySelectorAll("video[data-previewVideo-src][data-preview-group='" + group + "']");
            }
        } else {
            imgs = document.querySelectorAll("video[data-previewVideo-src]");
        }
        if (imgs.length) {
    ...
    proto.addImage = function(img) {
        ...
        if (img.__mui_img_data && img.__mui_img_data.src === src) { //已缓存且图片未变化
            ...
            var lazyload = img.getAttribute('data-previewVideo-src');
            if (!lazyload) {
                lazyload = src;
            }
            var imgObj = {
    ...
    proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
        ...
        var imgEl = itemEl.querySelector('video');
    ...
    $.previewVideo = function(options) {
        if (!previewImageApi) {
            previewImageApi = new PreviewVideo(options);
        }
        return previewImageApi;
    };
    $.disposePreviewVideo = function(options) {
        var prevdom = document.getElementById("__MUI_PREVIEWVIDEO");
