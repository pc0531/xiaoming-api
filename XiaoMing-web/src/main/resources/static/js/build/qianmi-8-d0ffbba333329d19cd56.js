...
var _scriptjs = __webpack_require__(243);
...
var Login = (function (_React$Component) {
    ...
    function Login(props) {
        ...
        this._writeUsername = function (e) {
        ...
        this._writePassword = function (e) {
            var password = e.target.value;
            _this._password = password;
        };

        this._normalLogin = function (me) {
            var username = me.state.username;
            if (!username) {
                mui.toast('请输入用户名');
                return;
            }
            var url = _constants2['default'].Host + '/oauth/login';
            var condition = {};
            var password = me._password;
            if (/^\s*$/.test(password)) {
                mui.toast('密码不能为空');
                return;
            }
            condition.userName = username;
            condition.logPassword = _utilRSA2['default'].encryptedString(password);
            _utilRequest2['default'].post(url, condition).then(function (res) {
                _utilCommonInfo2['default'].setLoginInfo(res);
            }).fail(function (res) {
                if (res.result == 'fail') {
                    mui.toast(res.msg);
                } else {
        ...
        value: function render() {
            return _react2['default'].createElement(
                ...
                _react2['default'].createElement(
                    ...
                    _react2['default'].createElement(
                        ...
                        _react2['default'].createElement(
                            ...
                            _react2['default'].createElement(
                                'button',
                                { id: 'pwdLoginButton', type: 'button', className: 'ex-login-btn' },
                                '登录'
                            ),
                            _react2['default'].createElement(
                                'a',
                                { href: 'javascript:void(0)', className: 'ex-login-way' },
                                '<< 验证码登录'
                            )
                        )
                    )
                ...
                _react2['default'].createElement(
                    ...
                    _react2['default'].createElement(
                        'a',
                        { href: 'javascript:void(0)', id: 'register' },
                        '立即注册'
                    )
                )
            );
